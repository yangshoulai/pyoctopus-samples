...
@pyoctopus.hyperlink(pyoctopus.link(pyoctopus.xpath("//figure/a/@href", multi=True), repeatable=False, priority=2))
class AlbumList:
    pass
...
@pyoctopus.hyperlink(
    pyoctopus.link(
        pyoctopus.xpath('//a[@class="page-numbers"]/@href', multi=True),
        repeatable=False,
        priority=2,
    )
)
...
if __name__ == "__main__":
    ...
    processors = [
        (
            pyoctopus.or_matcher(
                pyoctopus.url_matcher(r"^.*/pages/.*\?query-3-page=1$"),
                pyoctopus.url_matcher(r"^.*/search/.*/page/1$"),
            ),
            get_all_pages,
        ),
        (
            pyoctopus.or_matcher(
                pyoctopus.url_matcher(r".*/pages/.*\?query-3-page=\d+"),
                pyoctopus.url_matcher(r".*/search/.*/page/\d+"),
            ),
            pyoctopus.extractor(AlbumList),
        ),
        (
            pyoctopus.url_matcher(r".*/content/\d+/.*"),
            pyoctopus.extractor(AlbumDetails),
        ),
        (pyoctopus.url_matcher(r".*/content/\d+/.*"), resolve_img_urls),
        (
            pyoctopus.IMAGE,
            pyoctopus.downloader(os.path.expanduser("~/Downloads/4khd"), sub_dir_attr="name"),
        ),
    ]
