...
@pyoctopus.hyperlink(
    pyoctopus.link(
        pyoctopus.xpath('//div[@class="paragraph"]/div[@class="title"]/a/@href', multi=True),
        repeatable=False,
        priority=1,
    ),
...
@pyoctopus.hyperlink(
    pyoctopus.link(
        pyoctopus.xpath('//div[@class="intro"]/img/@src', multi=True), repeatable=False, priority=3, attr_props=["name"]
    )
)
class AlbumDetails:
...
if __name__ == "__main__":
    ...
    pyoctopus.new(
        ...
        sites=sites,
        threads=2,
        store=pyoctopus.sqlite_store(os.path.expanduser("~/Downloads/pyoctopus.db"), table="xiuirenwang"),
    ).start(seed)
